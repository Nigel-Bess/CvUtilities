...
RUN mkdir -p Fulfil.OrbbecUtils/lib/orbbecsdk/lib && \
    ...
    cp -r orbbec/OrbbecSDK_v2/build/linux_x86_64/lib/* \
          Fulfil.OrbbecUtils/lib/orbbecsdk/lib && \
    cp -r orbbec/OrbbecSDK_v2/build/linux_x86_64/bin/* \
...
RUN --mount=type=cache,target=/ccache \
    ...
    cmake -S . -B build \
        -DIS_CONTAINERIZED=1 \
        $( [ "$WITH_GUI" = "1" ] && echo -DOpenCV_DIR=/opt/opencv-gtk/lib/cmake/opencv4 ) \
        -DCMAKE_C_COMPILER_LAUNCHER=ccache \
        -DCMAKE_CXX_COMPILER_LAUNCHER=ccache && \
