...
function display_help() {
    echo "Usage: $0 -n <namespace> -d <deployment_name> <remote_path>"
    echo
    echo "Options:"
    ...
    echo "  <remote_path>   Remote path on the pod for which files will be listed."
    echo
    echo "Example:"
    echo "  $0 -n default -d my-deployment /path/on/pod"
    exit 1
}
...
if [[ -z "$1" ]]; then
    echo "Error: Missing required remote directory arguments."
    display_help
fi
...
kubectl exec -n $namespace $pod_name -- ls -l $remote_path
