...
if [ "" = "$PKG_Installed" ] ; then
  ...
  if [[ "$DO_RS_USB_PATCH" == "ON" ]] && echo "The script is about to patch the kernel for USB camera control" \
    ...
    patch_kernel_cutoff="5.13"
    kernel_sort="$(echo -e "$(uname -r)\n$patch_kernel_cutoff" | sort --version-sort | tail -n 1 )"
    if [[ "${kernel_sort}" == "${patch_kernel_cutoff}" ]] ; then
      echo -e "${BLU}Running standard patch on kernel $(uname -r), instead of $patch_kernel_cutoff updated patch.${NC}"
      ./scripts/patch-realsense-ubuntu-lts.sh
      EXIT_LOG "patch-realsense-ubuntu-lts" $?

    else
      echo -e "${BLU}Running new hw patch since host kernel version $(uname -r), exceeds cutoff $patch_kernel_cutoff${NC}"
      ./scripts/patch-realsense-ubuntu-lts-hwe.sh
      EXIT_LOG "patch-realsense-ubuntu-lts-hwe" $?
    fi
  else
