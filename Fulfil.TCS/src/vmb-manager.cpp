...
void VmbManager::HandleRequest(std::shared_ptr<DepthCameras::DcRequest> request, int remainingRetries){
    ...
    switch(type){
        case DispenseCommand::bag_release:
        {
            ...
            TCSResponse response(cmd_id, pkid, always_approve_for_release, 10, false, "Bay not found");
            if(cam == nullptr || dropoff_lane_id < 0){
                log_->Error("No TCS bay with id {}", dropoff_lane_id);
            }
            else{
                ...
                TCSPerception TCS_percep(lfb_generation);
                ...
                if (!std::filesystem::exists(directory_path)) {
                    if (std::filesystem::create_directories(directory_path)) {
                        ...
                        if (!saved_image_successfully) {
                            TCS_percep.success_code = TCSErrorCodes::UnspecifiedError;
                            TCS_percep.error_description = "Failed to save image successfully, likely the image was empty!";
                        }
                    }
                    else {
                        log_->Error("Failed to create Directory : {}", directory_path);
                        TCS_percep.success_code = TCSErrorCodes::UnspecifiedError;
                        TCS_percep.error_description = "Failed to create Directory : " + directory_path;
                    }
                }
                
                try {
                    // don't run the algorithm if the setup encountered issues
                    if (TCS_percep.success_code == TCSErrorCodes::Success)
                    {
                        log_->Info("About to run is_bot_ready_for_release");
                        int lfbId = request_json->value("Lfb_Id", 0);
                        TCS_percep.is_bot_ready_for_release(image, *pkid, cam->name_, lfbId, directory_path);
                        log_->Info("Results from is_bot_ready_for_release are success code: {}, is_bag_empty: {}, error_description: {}", 
                            TCS_percep.success_code, 
                            TCS_percep.is_bag_empty, 
                            TCS_percep.error_description);
                    ...
                    {
                        if (cam->camera_error_code != TCSErrorCodes::Success) {
                            TCS_percep.success_code = cam->camera_error_code;
                            TCS_percep.error_description = get_error_name_from_code((TCSErrorCodes)TCS_percep.success_code) + " -> " +  cam->camera_error_description;
                        } 
                        log_->Debug("TCSPerception success code is {}, Not going to proceed with is_bot_ready_for_release", TCS_percep.success_code);
                    }
                    
                    response = TCSResponse(cmd_id, pkid, always_approve_for_release, TCS_percep.success_code, TCS_percep.is_bag_empty, TCS_percep.error_description);
                }
                catch (const std::exception& e){
                    log_->Error("Exception caught in VmbManager. Issue handling bot release request: \n\t{}", e.what());
                    response = TCSResponse(cmd_id, pkid, always_approve_for_release, TCSErrorCodes::UnspecifiedError, false, e.what());
                }
            }
