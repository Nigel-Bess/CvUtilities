...
DispenseManager::DispenseManager(
    ...
    std::shared_ptr<fulfil::dispense::tray::TrayManager> tray_manager) : bay(bay),
                                                                         dispense_reader(dispense_man_reader),
                                                                         tray_config_reader(tray_config_reader),
    ...
    auto safe_get_tray_string_val =
            [&tray_dim_type = this->tray_dimension_type, &tray_config=this->tray_config_reader](
        auto key, std::string default_value) {
    ...
    Logger::Instance()->Info("Initializing socket network manager at configured port {} for machine {}", port,
                             this->machine_name);
    ...
    if (tray_session) {
        ...
        Logger::Instance()->Info("AGX specific ini file indicates to use tray config section {}",
                                 this->tray_dimension_type);
    ...
    if (LFB_session) {
        ...
        if (dispense_man_reader->GetBoolean(dispense_man_reader->get_default_section(), "live_visualize_drop", false)) {
            std::string live_viewer_path = this->dispense_reader->Get(dispense_man_reader->get_default_section(),
                                                                      "live_visualize_drop_path", "");
            if (live_viewer_path.empty()) {
                ...
                std::shared_ptr<GCSSender> gcs_sender = std::make_shared<GCSSender>(
                    this->cloud_media_bucket, cloud_directory);
                this->live_viewer = std::make_shared<LiveViewer>(live_viewer_path, gcs_sender);
                Logger::Instance()->Debug("Live Viewer on Drop for bay: {}, set up with base path: {}", bay,
                                          live_viewer_path);
            }
        }
    }
    ...
    std::shared_ptr<DispenseRequestParser> parser = std::make_shared<DispenseRequestParser>(
        std::make_shared<DispenseJsonParser>());
    std::shared_ptr<SocketManager> socket_manager = std::make_shared<SocketManager>(
        std::make_shared<SocketInformation>(port));

    this->network_manager = std::make_shared<SocketNetworkManager<std::shared_ptr<DispenseRequest> > >(
        socket_manager, parser);

    this->processing_queue = std::make_shared<ProcessingQueue<std::shared_ptr<DispenseRequest>, std::shared_ptr<
        DispenseResponse> > >();
...
void DispenseManager::handle_request_in_thread(std::shared_ptr<std::string> payload,
...
std::shared_ptr<fulfil::dispense::drop::DropResult> DispenseManager::handle_drop_target(
    ...
    auto timer = fulfil::utils::timing::Timer(
        "DispenseManager::handle_drop_target for " + this->machine_name + " request " + PrimaryKeyID);
    ...
    if (!this->LFB_session) {
        ...
        return std::make_shared<DropResult>(details->request_id, DropTargetErrorCodes::AlgorithmFail_Bypass,
                                            "No LFB Session: Bouncing Drop Camera Drop Target");
        // Todo: move to throw/catch format, log data
    }
    ...
    if (details->bag_id.compare(this->bag_id) != 0) {
        ...
        return std::make_shared<DropResult>(details->request_id,
                                            ...
                                            "` while Bag ID in the dispense manager is: `" + this->bag_id + "`");
        //Todo: move to throw/catch format, log data
    }
    ...
    if (!this->dispense_reader->GetBoolean("device_specific", "use_advanced_damage_criteria", false)) {
        ...
        Logger::Instance()->Info("Using advanced damage criteria, damage code for this item: {}",
                                 details->item_damage_code);
    }
    ...
    std::shared_ptr<fulfil::dispense::drop::DropResult>
            drop_result = this->drop_manager->handle_drop_request(request_json, details, base_directory,
                                                                  time_stamp_string,
    ...
    if (drop_result->success_code != DropTargetErrorCodes::Success and drop_result->success_code !=
        DropTargetErrorCodes::EmptyBagNotEmpty) {
        if (this->live_viewer != nullptr) {
            ...
            std::string specific_error_message = get_error_name_from_code(
                (DropTargetErrorCodes) drop_result->success_code);
            message->push_back(specific_error_message);
            std::cout << specific_error_message << std::endl;
            this->live_viewer->update_image(live_viewer->get_blank_visualization(), ViewerImageType::Info, PrimaryKeyID,
                                            true, message);
    ...
    } else //case for valid drop target result without error
    {
        if (drop_result->Rotate_LFB) {
            Logger::Instance()->Info(
                "Drop Target Result recommends to rotate LFB; caching to disallow further rotation on this dispense");
...
std::shared_ptr<PostLFRResponse> DispenseManager::handle_post_LFR(std::shared_ptr<std::string> PrimaryKeyID,
                                                                  ...
                                                                  std::shared_ptr<nlohmann::json> request_json) {
    auto timer = fulfil::utils::timing::Timer(
        "DispenseManager::handle_post_LFR for " + this->machine_name + " request " + *PrimaryKeyID);
    ...
    this->drop_manager->cached_post_container = nullptr;
    //reset to nullptr before processing begins, in case encounter errors and prepostcomparison is not possible
    this->drop_manager->cached_post_request = nullptr;
    this->bot_already_rotated_for_current_dispense = false;
    //reset this flag because handle_post_dispense indicates a dispense just happened

    std::shared_ptr<std::string> base_directory = this->create_datagenerator_basedir();
    //call post-drop algorithm
    std::shared_ptr<PostLFRResponse> response = (this->LFB_session)
                                                    ? this->drop_manager->handle_post_LFR(
                                                        request_json,
        ...
        if (comparison_flag == 1) {
            ...
            if (detection_results.first != -1) {
                ...
                Logger::Instance()->Debug(
                    "Bag fit check: found {} products of interest that will no longer fit in this bag",
                    products_to_overflow.size());
...
void DispenseManager::handle_start_lfb_video(std::shared_ptr<std::string> PrimaryKeyID) {
    std::string base_directory = this->dispense_reader->Get(dispense_reader->get_default_section(),
                                                            "vid_gen_base_buffer_dir");
    if (!this->LFB_session or !this->lfb_upload_generator) {
        Logger::Instance()->Warn("Bouncing Bay {} Drop Camera Video; Either missing session or generator",
                                 this->machine_name);
        return;
    }
    ...
    if (!this->lfb_upload_generator->stop_video_saving) // Move check to video uploader
    {
        Logger::Instance()->Warn(
            "Previous Drop Camera video may still be running! Will send stop signal now and wait 250ms before proceeding");
    ...
    std::thread lfb_video_recorder_thread(
        [this, PrimaryKeyID, base_directory, sender]() //start new lfb_video_recorder_thread for saving LFB video
        {
...
void DispenseManager::handle_start_tray_video(std::shared_ptr<std::string> PrimaryKeyID) {
    ...
    if (!this->tray_session or !this->tray_upload_generator) {
        Logger::Instance()->Warn("Bouncing Bay {} Tray Camera Video; Either missing session or generator",
                                 this->machine_name);
        return;
    }
    ...
    {
        Logger::Instance()->Warn(
            "Previous Tray video may still be running! Will send stop signal now and wait 250ms before proceeding");
    ...
    std::thread tray_video_recorder_thread(
        [this, PrimaryKeyID, base_directory, sender]() //start new thread for saving tray video
        {
...
void DispenseManager::handle_stop_tray_video(int delay) {
    ...
    //TODO: this value should be based on the fps value for the video generator (e.g. 5fps = 200ms, 300ms > 200ms)
    usleep(400000);
    //sleep for 0.4s long enough for stop flag to take effect, even with framerate pause in save_many_frames
...
void DispenseManager::handle_stop_request(std::shared_ptr<std::string> command_id) {
    std::shared_ptr<ProcessingQueuePredicate<std::shared_ptr<DispenseRequest> > > predicate = std::make_shared<
        DispenseProcessingQueuePredicate>(command_id);
    this->processing_queue->purge_queue(predicate);
}

std::shared_ptr<FloorViewResponse> DispenseManager::handle_floor_view(std::shared_ptr<std::string> PrimaryKeyID,
                                                                      std::shared_ptr<std::string> command_id,
                                                                      std::shared_ptr<nlohmann::json> request_json) {
    auto timer = fulfil::utils::timing::Timer(
        "DispenseManager::handle_floor_view for " + this->machine_name + " with PKID " + *PrimaryKeyID);
    ...
    Logger::Instance()->Info("Finished handling Floor View command. Result: "
                             "Bay: {} PKID: {} Anomaly Present: {}, Item on Ground: {}, Bots in Image: {}",
                             this->machine_name, *PrimaryKeyID, anomaly_detected, item_on_ground,
                             floor_analysis_confidence_score);
    return floor_result;
}

results_to_vlsg::TrayValidationCounts dispatch_to_count_api(
    const std::shared_ptr<fulfil::dispense::tray::TrayManager> &tray_manager,
...
DispenseManager::handle_item_edge_distance(std::shared_ptr<std::string> command_id,
                                           std::shared_ptr<nlohmann::json> request_json) {
    request_from_vlsg::TrayRequest single_lane_val_req = request_json->get<request_from_vlsg::TrayRequest>();
    auto timer = fulfil::utils::timing::Timer(
        "DispenseManager::handle_item_edge_distance for " + this->machine_name + " request " + single_lane_val_req.
        m_context.get_id_tagged_sequence_step());
    Logger::Instance()->Debug("Handling {} Dispense Lane Processing {} for Bay: {}",
                              single_lane_val_req.get_sequence_step(), single_lane_val_req.get_primary_key_id(),
                              this->machine_name);
    ...
    if (this->live_viewer) {
        this->live_viewer->update_image(std::make_shared<cv::Mat>(this->tray_session->grab_color_frame()), image_code,
                                        single_lane_val_req.get_primary_key_id(), true);
    }
    auto saved_images_base_directory = this->dispense_reader->Get(this->dispense_reader->get_default_section(),
                                                                  "data_gen_image_base_dir");
    ...
    Logger::Instance()->Info("Finished handling Single Lane Dispense command. Result: "
                             "Bay: {} PKID: {} Distance {}", this->machine_name,
                             single_lane_val_req.get_primary_key_id(), tray_result->get_fed_value());
    return tray_result;
}
...
DispenseManager::handle_tray_validation(std::shared_ptr<std::string> command_id,
                                        std::shared_ptr<nlohmann::json> request_json) {
    auto make_null_tray_validation_result = [command_id]() {
        // by value since it's a fuckin ptr
    ...
    auto timer = fulfil::utils::timing::Timer(
        "DispenseManager::handle_tray_validation for " + this->machine_name + " request " + tray_validation_request.
        get_primary_key_id());
    Logger::Instance()->Debug("Handling Tray Validation Command {} for Bay: {}",
                              tray_validation_request.get_primary_key_id(), this->machine_name);
    ...
    auto send_color_mat = [&](double scale_resize) {
        try {
            if (this->tray_manager->save_tray_audit_image(tray_validation_request.m_context, local_base_path,
                                                          scale_resize)) {
                return this->tray_manager->upload_tray_data(tray_validation_request.m_context, local_base_path,
                                                            GCSSender(this->cloud_media_bucket, this->store_id));
            }
        } catch (const std::exception &e) {
    ...
    DataGenerator tray_validation_data_generator = tray_manager->build_tray_data_generator(
        request_json,
        tray_manager->make_default_datagen_path(saved_images_base_directory, tray_validation_request) /
        tray_validation_request.get_sequence_step());
    ...
    /** run algorithms **/
    auto dispatch_to_count_api = [&tray_validation_request, &saved_images_base_directory, tm=tray_manager
            ](auto center_pixels) {
    ...
    auto do_all_tray_processing = [&](auto save_function, auto run_function) {
        try {
            ...
            Logger::Instance()->Info(
                "Request {} has tallest item tray buffer of {}, and returned height of tray is {} with the confidence {}."
                " After adjusting for tray inset, error is {} and detected height is {:0.4f} of expected height.",
                tray_validation_request.get_primary_key_id(), exp_max, max_height_detected_in_tray,
                count_response.m_height_info.m_confidence,
    ...
    if (send_color_mat(resize_factor) < 0) {
        Logger::Instance()->Error("Issue uploading Tray Validation audit data for {}",
...
int DispenseManager::handle_update_state(std::shared_ptr<std::string> PrimaryKeyID,
                                         std::shared_ptr<nlohmann::json> request_json) {
...
std::string DispenseManager::handle_get_state(std::shared_ptr<std::string> PrimaryKeyID,
                                              std::shared_ptr<nlohmann::json> request_json) {
...
int DispenseManager::handle_pre_LFR(std::shared_ptr<std::string> PrimaryKeyID,
                                    std::shared_ptr<nlohmann::json> request_json) {
    auto timer = fulfil::utils::timing::Timer(
        "DispenseManager::handle_pre_LFR for " + this->machine_name + " request " + *PrimaryKeyID);
    ...
    try {
        ...
        std::shared_ptr<MarkerDetectorContainer> container = this->drop_manager->searcher->get_container(
            this->drop_manager->mongo_bag_state->raw_mongo_doc->Config,
            mock_session_pre,
         ...
         */
        if (this->live_viewer != nullptr) {
            this->live_viewer->update_image(this->LFB_session->get_color_mat(), ViewerImageType::LFB_Pre_Dispense,
                                            *PrimaryKeyID);
        }
...
std::shared_ptr<std::string> fulfil::dispense::DispenseManager::create_datagenerator_basedir() {
    std::shared_ptr<std::string> base_directory = std::make_shared<std::string>(
        this->dispense_reader->Get(dispense_reader->get_default_section(), "data_gen_image_base_dir", ""));
    if (base_directory->length() == 0) {
        Logger::Instance()->Fatal("Could not find data_gen_image_base_dir value in section {} of main_config.ini!",
                                  dispense_reader->get_default_section());
...
fulfil::dispense::DispenseManager::handle_side_dispense_target(std::shared_ptr<std::string> request_id,
    ...
    auto data_fs_path = make_media::paths::add_basedir_date_suffix_and_join(
                            this->dispense_reader->Get(this->dispense_reader->get_default_section(),
                                                       "data_gen_image_base_dir"),
                            "Side_Bag_Camera/") / (*request_json)["Primary_Key_ID"].get<std::string>();
    data_fs_path /= "Side_Dispense_Target";
    this->LFB_session->refresh();
    auto data_generator = DataGenerator(this->LFB_session, std::make_unique<std::string>(data_fs_path.string()),
                                        request_json);
