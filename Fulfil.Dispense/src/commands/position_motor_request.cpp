...
using fulfil::dispense::commands::PositionMotorRequest;
using fulfil::dispense::commands::CodeResponse;
using fulfil::dispense::commands::DispenseResponse;
using fulfil::utils::Logger;


PositionMotorRequest::PositionMotorRequest(std::shared_ptr<std::string> command_id, std::shared_ptr<std::string> PrimaryKeyID, std::shared_ptr<nlohmann::json> request_json)
{
  /**
...
std::shared_ptr<DispenseResponse> PositionMotorRequest::execute()
{
  if(!this->delegate.expired())
  {
    std::shared_ptr<DispenseRequestDelegate> tmp_delegate = this->delegate.lock();

    int error_code = tmp_delegate->handle_position_motor(this->PrimaryKeyID, this->request_json);

    return std::make_shared<CodeResponse>(this->command_id, error_code);
  }
  else
  {
    Logger::Instance()->Error("Command Delegate Expired; Vars: PositionMotorRequest");
