...
class DropResult
{
 ...
 public:
   ...
   */
  explicit DropResult(std::shared_ptr<std::string> request_id, int error_code);
  ...
  DropResult(std::shared_ptr<fulfil::utils::Point3D> drop_center, float max_Z, bool Rotate_LFB, bool LFB_Currently_Rotated,
             bool Swing_Collision_Expected, std::shared_ptr<std::string> request_id, int success_code = 0); /**
