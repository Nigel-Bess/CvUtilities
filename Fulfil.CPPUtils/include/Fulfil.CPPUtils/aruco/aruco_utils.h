...
namespace fulfil
{
    namespace utils
    {
        namespace aruco
        {
            ...
            struct ImageMarkers
            {
                ImageMarkers(std::shared_ptr<std::vector<std::shared_ptr<std::vector<std::shared_ptr<cv::Point2f>>>>> markers, std::shared_ptr<std::vector<int>> ids) : markers(markers), ids(ids) {};

                std::shared_ptr<std::vector<std::shared_ptr<std::vector<std::shared_ptr<cv::Point2f>>>>> markers;
                std::shared_ptr<std::vector<int>> ids;
            };

            struct BestMarkers {
                BestMarkers(std::shared_ptr<ImageMarkers> matches, double distance_score) : matches(matches), distance_score(distance_score) {};
                /**
                 * Array of matching markers in target image
                 */
                std::shared_ptr<ImageMarkers> matches;
                /**
                 * Sum of distances over point counts plus a missing IDs penalty between candidate and target markers
                 */
            ...
            struct HomographyResult {
                HomographyResult(
                std::shared_ptr<BestMarkers> imgMarkers,
            ...
            private:
                 ...
                 * in `candidate->ids`.
                 */
                double calculateMarkerAvgDist(std::shared_ptr<ImageMarkers> candidate, std::shared_ptr<std::vector<std::shared_ptr<cv::Point2f>>> markerCorners, int markerId);
                 ...
                 */
                std::shared_ptr<BestMarkers> findBestMarkers(cv::Mat targetImage, std::shared_ptr<ImageMarkers> candidate, std::shared_ptr<ImageMarkers> instance);
            ...
            public:
                 ...
                 */
                static double distance(const std::shared_ptr<cv::Point2f> p1, const std::shared_ptr<cv::Point2f> p2)
                {
                 ...
                 * IDs are consistent in their spacial relations across both images!  Returned vector is all possible
                 * homog results ranked by best match first.
                 */
                ...
                std::shared_ptr<HomographyResult> to_homog_result(cv::Mat &targetImage, std::string name, std::shared_ptr<BestMarkers> bestMatches, std::string debugDrawDir);
