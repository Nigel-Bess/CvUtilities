...
using fulfil::dispense::commands::RepackError;
using fulfil::dispense::commands::RepackErrorCodes;

// note: message param is optional and is default empty
RepackError::RepackError(RepackErrorCodes status_code, const std::string& description) {
    this->status_code = status_code;
    this->status_name = get_error_name_from_code(status_code);
    this->message = "Repack Empty Bag Algorithm error " + std::to_string(this->status_code) + ": " + this->status_name;
    this->description = description;
    // if the message parameter is non-empty, append
    if (!this->description.empty()) { this->message = this->message + " - " + this->description; }
}

const char* RepackError::what() const noexcept { return this->message.data(); }